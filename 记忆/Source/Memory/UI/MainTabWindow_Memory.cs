...
namespace RimTalk.Memory.UI
{
    ...
    /// ⭐ v3.3.22: Performance optimization - Cached filter results
    /// </summary>
    public partial class MainTabWindow_Memory : MainTabWindow
    {
        ...
        // ⭐ v3.3.22: PERFORMANCE - Cached filtered memories
        ...
        private void DrawPawnSelector(Rect rect)
        {
            ...
            if (Widgets.ButtonText(rect, label))
            {
                ...
                foreach (var pawn in colonists)
                {
                    ...
                    { 
                        ...
                        filtersDirty = true; // ⭐ v3.3.22: Invalidate cache when pawn changes
                    }));
                }
            ...
            if (selectedPawn == null && colonists.Count > 0)
            {
                ...
                filtersDirty = true; // ⭐ v3.3.22: Invalidate cache on auto-select
            }
        }
        ...
        private void DrawColoredCheckbox(Rect rect, string label, ref bool value, Color color, MemoryLayer? rightClickLayer)
        {
            ...
            // ⭐ v3.3.22: Detect checkbox changes and invalidate cache
            bool oldValue = value;
            ...
            // ⭐ v3.3.22: Set dirty flag if value changed
            if (oldValue != value)
            {
                filtersDirty = true;
            }
        ...
        private float DrawTypeFilters(Rect parentRect, float startY)
        {
            ...
            {
                if (filterType != null) // ⭐ v3.3.22: Only set dirty if actually changing
                {
                    filterType = null;
            ...
            {
                if (filterType != MemoryType.Conversation) // ⭐ v3.3.22: Only set dirty if actually changing
                {
            ...
            {
                if (filterType != MemoryType.Action) // ⭐ v3.3.22: Only set dirty if actually changing
                {
        ...
        private void DrawMemoryCard(Rect rect, MemoryEntry memory)
        {
            ...
            if (Widgets.ButtonImage(editButtonRect, TexButton.Rename))
            {
                if (currentMemoryComp != null)
                {
                    ...
                    filtersDirty = true; // ⭐ v3.3.22: Invalidate cache when opening edit dialog (content might change)
                }
                clickedOnButton = true;
                Event.current.Use();
            }
        ...
        private void SummarizeMemories(List<MemoryEntry> targetMemories)
        {
            ...
            Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                ...
                delegate
                {
                    ...
                    selectedMemories.Clear();
                    filtersDirty = true; // ⭐ v3.3.22: Invalidate cache after memories change
        ...
        private void DeleteMemories(List<MemoryEntry> targetMemories)
        {
            ...
            Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                ...
                delegate
                {
                    ...
                    selectedMemories.Clear();
                    filtersDirty = true; // ⭐ v3.3.22: Invalidate cache after memories deleted
        ...
        /// <summary>
        /// ⭐ v3.3.22: Get filtered memories with caching
        /// Returns cached result if filters haven't changed
        /// </summary>
        private List<MemoryEntry> GetFilteredMemories()
        {
            // If cache is invalid or dirty, rebuild it
            if (filtersDirty || cachedFilteredMemories == null)
            {
                cachedFilteredMemories = RebuildFilteredMemories();
                filtersDirty = false;
            }
        ...
        /// <summary>
        /// ⭐ v3.3.22: Rebuild filtered and sorted memory list
        /// This is the expensive operation that we cache
        /// </summary>
        private List<MemoryEntry> RebuildFilteredMemories()
        {
            if (currentMemoryComp == null)
            ...
            try
            {
                ...
                Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation(
                    ...
                    delegate
                    {
                        ...
                        filtersDirty = true; // ⭐ v3.3.22: Invalidate cache after import
