    ...
    public class FourLayerMemoryComp : ThingComp
    {
        ...
        public override void PostExposeData()
        {
            ...
            if (Scribe.mode == LoadSaveMode.PostLoadInit)
            {
                ...
                // ⭐ v3.3.22: Restore interrupted AI summarization tasks
                RestoreInterruptedAITasks();
            }
        }
        
        /// <summary>
        /// ⭐ v3.3.22: Restore AI summarization tasks that were interrupted by save/load
        /// Finds memory entries with "待AI更新" tag and re-triggers AI summarization
        /// </summary>
        private void RestoreInterruptedAITasks()
        {
            // Only restore if AI summarization is enabled
            if (!RimTalkMemoryPatchMod.Settings.useAISummarization || !AI.IndependentAISummarizer.IsAvailable())
            {
                // AI is disabled, clean up pending tags
                foreach (var memory in eventLogMemories)
                {
                    if (memory.tags.Contains("待AI更新"))
                    {
                        memory.RemoveTag("待AI更新");
                        memory.notes = "AI 总结已取消（AI功能未启用）";
                    }
                }
                return;
            }
            ...
            // Search for interrupted tasks in eventLogMemories
            foreach (var memory in eventLogMemories)
            {
                if (memory.tags.Contains("待AI更新"))
                {
                    // This memory was waiting for AI update when the game was saved
                    // Re-trigger the AI summarization
                    
                    // We need to reconstruct the original memories that were summarized
                    // Since they were already cleared, we'll use the existing simple summary
                    // and ask AI to improve it
                    
                    TriggerAISummaryRefresh(memory, pawn);
                    restoredCount++;
                }
            }
            
            // Also check archiveMemories
            ...
            if (restoredCount > 0 && Prefs.DevMode)
            {
                Log.Message($"[Memory] {pawn.LabelShort}: Restored {restoredCount} interrupted AI summarization tasks");
            }
        }
        
        /// <summary>
        /// ⭐ v3.3.22: Trigger AI summary refresh for a memory entry
        /// Helper method used by both initial summarization and restore logic
        /// </summary>
        private void TriggerAISummaryRefresh(MemoryEntry summaryEntry, Pawn pawn)
        {
            // Create a single-item list for the summarizer
            // (We don't have the original memories, so we'll ask AI to refine the simple summary)
            var pseudoMemories = new List<MemoryEntry> { summaryEntry };
            
            string cacheKey = AI.IndependentAISummarizer.ComputeCacheKey(pawn, pseudoMemories);
            ...
            AI.IndependentAISummarizer.RegisterCallback(cacheKey, (aiSummary) =>
            {
                if (!string.IsNullOrEmpty(aiSummary))
                {
                    ...
                    summaryEntry.RemoveTag("待AI更新");
                    summaryEntry.AddTag("AI总结");
                    summaryEntry.notes = "AI 总结已于后台完成并自动更新。";
                    
                    if (Prefs.DevMode)
                    {
                        Log.Message($"[Memory] {pawn.LabelShort}: AI summary restored for entry: {summaryEntry.content.Substring(0, Math.Min(50, summaryEntry.content.Length))}...");
                    }
                }
            });
            
            // Determine prompt template based on layer
            string promptTemplate = summaryEntry.layer == MemoryLayer.Archive ? "deep_archive" : "daily_summary";
            
            AI.IndependentAISummarizer.SummarizeMemories(pawn, pseudoMemories, promptTemplate);
            
            // Update notes to indicate task is being retried
            summaryEntry.notes = "AI 总结正在恢复中（存档加载后重新触发）...";
        }
        ...
        public void DailySummarization()
        {
            ...
            foreach (var typeGroup in byType)
            {
                ...
                // ⭐ v3.3.22: Use helper method for AI summarization
                TriggerAISummarization(summaryEntry, pawn, memories, "daily_summary");
        ...
        /// <summary>
        /// ⭐ v3.3.22: Trigger AI summarization for a memory entry (shared logic)
        /// Used by DailySummarization, ManualSummarization, and task restoration
        /// </summary>
        private void TriggerAISummarization(MemoryEntry summaryEntry, Pawn pawn, List<MemoryEntry> sourceMemories, string promptTemplate)
        {
            if (!RimTalkMemoryPatchMod.Settings.useAISummarization || !AI.IndependentAISummarizer.IsAvailable())
                return;
            
            string cacheKey = AI.IndependentAISummarizer.ComputeCacheKey(pawn, sourceMemories);
            ...
            AI.IndependentAISummarizer.SummarizeMemories(pawn, sourceMemories, promptTemplate);
